...
def getChar(val):
    ...
    if index > 0 and sorted_weights[index][1] + sorted_weights[index - 1][1] > 2 * val:
        index -= 1
...
outputTextFile = 'ycy.txt'
outputImageFile = 'ycy_ascii.gif'
...
asc_image = Image.new('L', (int(base.size[0] * sizes[2]), int(base.size[1] * sizes[2])), 255)
